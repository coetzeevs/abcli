...
pub static SECRETS: Lazy<SecretsManager> = Lazy::new(|| {
    let (store_path, key_path) = match std::env::var("ENV").as_ref().map(|s| s.as_str()) {
        Ok("DEV") => (
            "./secure/secrets.dev.json",
            Path::new("./secure/secrets.dev.key"),
        ),
        Ok("STG") => (
            "./secure/secrets.stg.json",
            Path::new("./secure/secrets.stg.key"),
        ),
        Ok("PRD") => (
            "./secure/secrets.prod.json",
            Path::new("./secure/secrets.prod.key"),
        ),
        _ => (
            "./secure/secrets.dev.json",
            Path::new("./secure/secrets.dev.key"),
